...
public class PlayerInput : MonoBehaviour {
    ...
    void Update()
    {
        ...
        //Change Orientation
        if ((Input.GetKeyDown(KeyCode.A) || Input.GetAxisRaw("Controller_LTrigger") == 1) && !PlayerMovement.isClimbing)
        {
            camMovement.RotateTo(CameraMovement.Rotations.lLeft);
            rotatedCam = true;
        }
        else if ((Input.GetKeyDown(KeyCode.D) || Input.GetAxisRaw("Controller_RTrigger") == 1) && !PlayerMovement.isClimbing)
        {
        ...
        if (Input.GetKeyDown(KeyCode.U) || Input.GetButtonDown("Controller_Y")) BuildScript.SetBuildMode();
        ...
        if (!BuildScript.allowBuild)
        {
            ...
            //Player movement
                ...
                if ((platformMovement.isPlayer() && PlatformMovement.moving == false) || !platformMovement.isPlayer())
                {
        ...
        else
        {
            ...
            else if (Input.GetKeyDown(KeyCode.K) || Input.GetAxisRaw("Controller_RJoystickV") < -sensibility) GhostMovement.MoveTo(AxisMovement.Directions.up);
            else if (Input.GetKeyDown(KeyCode.M) || Input.GetAxisRaw("Controller_RJoystickV") > sensibility) GhostMovement.MoveTo(AxisMovement.Directions.down);


            //Building Inputs
            if (Input.GetKeyDown(KeyCode.P) || Input.GetButtonDown("Controller_A")) BuildScript.BuildCubeOrRamp();
            //else if (Input.GetKeyDown(KeyCode.O) || Input.GetButtonDown("Controller_B")) BuildScript.Destroy();
            else if (Input.GetKeyDown(KeyCode.I) || Input.GetButtonDown("Controller_X")) BuildScript.SwitchBuilding();
            else if (Input.GetKeyDown(KeyCode.V) || Input.GetButtonDown("Controller_LBumper")) BuildScript.RotateRamp(false);
            else if (Input.GetKeyDown(KeyCode.B) || Input.GetButtonDown("Controller_RBumper")) BuildScript.RotateRamp(true);
        }
